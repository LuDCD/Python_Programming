...
all letters between the first and
last letter of the first name must be replaced by 5 asterisks '*'.
...
The last 10 digits make up the local number,最后十个数字为本地号码
...
as well as extra dashes or plus signs not part of the above formatting scheme
should be removed.
...
class Solution:
    def maskPII(self, S):
        ...
        if '@' in S:    # 是邮箱
            ...
            for e in S:
                if e not in {'+', '-', '(', ')', ' '}:
                    clearNumStr = clearNumStr + e

            localNumStr = '***-***-' + clearNumStr[-4:]

            leg = len( clearNumStr )
            if leg > 10:
                strRe = '+' + '*'*(leg-10) + '-' + localNumStr
            else:
                strRe = localNumStr
